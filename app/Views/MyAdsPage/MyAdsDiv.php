                ...
                <div class="col-md-8 col-xl-8 d-flex flex-column justify-content-around">
                    ...
                    <div class="d-flex justify-content-end mb-2">
                        <?php if ($ad->getProductStatus() === Status::Available) { ?>
                            <button class="btn btn-primary mx-2" onclick="btnMarkAsSoldClicked(<?= $ad->getId() ?>)">Mark As Sold</button>
                        <?php } else { ?>
                            <button class="btn btn-primary mx-2" disabled>Mark As Sold</button>
                        <?php } ?>
                        <button class="btn btn-secondary mx-2" data-bs-toggle="modal" data-bs-target="#editModal" onclick="editAdButtonClicked('<?= $ad->getId() ?>','<?= $ad->getProductImageURI() ?>','<?= $ad->getProductName() ?>','<?= addslashes($ad->getproductDescription()) ?>','<?= $ad->getproductPrice() ?>')">
